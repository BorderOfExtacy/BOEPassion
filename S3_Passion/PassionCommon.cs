...
namespace Passion.S3_Passion.Utilities
{
    public class PassionCommon
    {
        ...
        public class SimPart
        {
            ...
            public abstract class Types
            {
                public abstract class Penis
                {
                    ...
                    public static FacialBlendData ErectBlend
                    {
                        get
                        {
                            if (_mErectBlend == null)
                            {
                                try
                                {
                                    _mErectBlend = new FacialBlendData(
                                        new BlendUnit(new ResourceKey(4287645100653294805uL, 3039776853u, 0u)));
                                }
                                catch
                                {
                                    // ignored
                    ...
                    public static FacialBlendData FemaleErectBlend
                    {
                        get
                        {
                            if (_mFemaleErectBlend == null)
                            {
                                try
                                {
                                    _mFemaleErectBlend = new FacialBlendData(
                                        new BlendUnit(new ResourceKey(4029370978840255872uL, 3039776853u, 0u)));
                                }
                                catch
                                {
                                    // ignored
                    ...
                    public static FacialBlendData ForeskinBlend
                    {
                        get
                        {
                            if (_mForeskinBlend == null)
                            {
                                try
                                {
                                    _mForeskinBlend = new FacialBlendData(
                                        new BlendUnit(new ResourceKey(17118777923511104575uL, 3039776853u, 0u)));
                                }
                                catch
                                {
                                    // ignored
                    ...
                    {
                        get { return ErectBlend != null || FemaleErectBlend != null; }
                    }
                ...
                public class Height
                {
                    ...
                    public static FacialBlendData Blend
                    {
                        get
                        {
                            if (MBlend == null)
                            {
                                try
                                {
                                    MBlend = new FacialBlendData(new BlendUnit(
                                        ResourceKey.FromString("FBLN 0xB52F5055-0x00000000-0xF9E284139E1F88CF")));
                                }
                                catch
                                {
                                    // ignored
                    ...
                    public static float GetModifier(Sim sim)
                    {
                        ...
                        if (sim != null && Blend != null)
                        {
                            try
                            {
                                ...
                                {
                                    outfit = sim.SimDescription.GetOutfit(sim.CurrentOutfitCategory,
                                        sim.CurrentOutfitIndex);
                                    if (outfit != null && outfit.Blends != null)
                                    {
                    ...
                    private static Vector3 ApplyMultipliers(Sim sim, Vector3 variance, bool applyX, bool applyY,
                        bool applyZ)
                    {
                ...
                try
                {
                    ...
                    for (int i = 0; i < items.Length; i++)
                    {
                        if (items[i] != null)
                        {
                            ...
                            {
                                simBuilder.SetFacialBlend(items[i].Blend.mBlend2.GetKey(),
                                    Math.Abs(Math.Min(items[i].Value, 0f)));
                            }
                        }
                    }
                    ...
                    SimOutfit outfit2 =
                        new SimOutfit(
                            simBuilder.CacheOutfit(sim.FirstName + sim.LastName + category.ToString() + index));
        ...
        {
            get { return RandomUtil.GetRandomObjectFromList(ReactionsPositive); }
        }

        public static ReactionTypes RandomReactionNeg
        {
            get { return RandomUtil.GetRandomObjectFromList(ReactionsNegative); }
        }
        ...
        {
            get { return string.IsNullOrEmpty(_mBufferedMessageChunk); }
        }
        ...
        public static void Message(string message, StyledNotification.NotificationStyle style, Sim speaker, Sim target)
        {
            if (speaker != null)
            {
                if (target != null)
                {
                    StyledNotification.Show(new StyledNotification.Format(message, speaker.ObjectId, target.ObjectId,
                        style));
                }
                else
                {
        ...
        public static void Impregnate(Sim mom, Sim dad)
        {
            ...
            pregnancy.PreggersAlarm = mom.AddAlarmRepeating(1f, TimeUnit.Hours, pregnancy.HourlyCallback, 1f,
                TimeUnit.Hours, "Hourly Pregnancy Update Alarm", AlarmType.AlwaysPersisted);
        ...
        private static void CleanMoodlets(Sim sim, List<BuffNames> buffs)
        {
            try
            {
                if (sim == null || sim.HasBeenDestroyed || buffs == null || buffs.Count <= 0 ||
                    !sim.BuffManager.HasAnyElement(buffs.ToArray()))
                {
                    return;
                }
