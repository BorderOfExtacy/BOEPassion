...
namespace S3_Passion
{
	...
	{
		[DoesntRequireTuning]
		public sealed class Definition : InteractionDefinition<Sim, Sim, VampireBitesToKill>, IHasTraitIcon, IHasMenuPathIcon
		{
			...
			{
				return Localization.LocalizeString(target.IsFemale, "Vampire Bites To Kill", target.SimDescription, actor.SimDescription);
			}
			...
			{
				return GameUtils.IsInstalled(ProductVersion.EP7) && target != a && !a.LotCurrent.IsWorldLot && !target.IsActiveSim && !target.SimDescription.IsServicePerson && !target.Posture.Satisfies(CommodityKind.SwimmingInPool, null) && !target.SimDescription.IsGhost && !target.SimDescription.ChildOrBelow && !target.SimDescription.IsPet && a.SimDescription.IsVampire && a.TraitManager.HasElement(TraitNames.SuperVampire) && !a.SimDescription.ChildOrBelow && !a.Posture.Satisfies(CommodityKind.SwimmingInPool, null) && Passion.Settings.VampireInteractions;
			}
		}
		...
		public override bool Run()
		{
			...
			if (BeginSocialInteraction(new SocialInteractionB.Definition(), true, 0.75f, true))
			{
				...
				{
					Actor.BuffManager.AddElement(BuffNames.DrankFromAFairy, 1350f, Origin.FromReceivingVampireNutrients);
				}
				...
				if ((!Target.SimDescription.IsVampire && !Target.SimDescription.TraitManager.HasElement(TraitNames.Evil) && !Actor.SimDescription.TraitManager.HasElement(TraitNames.Evil)) || (!Target.SimDescription.IsWerewolf && !Target.SimDescription.TraitManager.HasElement(TraitNames.Evil) && !Actor.SimDescription.TraitManager.HasElement(TraitNames.Evil)))
				{
					SetTraits(Actor);
					if (actor != null)
					{
						...
						{
							if (!lotCurrent.LotCurrent.IsWorldLot && sim.SimDescription != actor.SimDescription && !sim.SimDescription.IsPet && !sim.SimDescription.IsWildAnimal)
							{
								Relationship relationship = Relationship.Get(actor.SimDescription, sim.SimDescription, true);
								if (relationship != null)
								{
									relationship.MakeAcquaintances();
									Relationship relationship2 = Relationship.Get(actor.SimDescription, sim.SimDescription, true);
				...
				else
				{
					if (Actor.SimDescription.TraitManager.HasElement(TraitNames.Evil))
					{
						if (actor != null)
						{
							...
							{
								if (!lotCurrent.LotCurrent.IsWorldLot && sim2.SimDescription != actor.SimDescription && !sim2.SimDescription.IsPet && !sim2.SimDescription.IsWildAnimal)
								{
									Relationship relationship3 = Relationship.Get(actor.SimDescription, sim2.SimDescription, true);
									if (relationship3 != null)
									{
										relationship3.MakeAcquaintances();
										Relationship relationship4 = Relationship.Get(actor.SimDescription, sim2.SimDescription, true);
					...
					if ((Target.SimDescription.IsVampire && Target.SimDescription.TraitManager.HasElement(TraitNames.Evil) && !Actor.SimDescription.TraitManager.HasElement(TraitNames.Evil)) || (Target.SimDescription.IsWerewolf && Target.SimDescription.TraitManager.HasElement(TraitNames.Evil) && !Actor.SimDescription.TraitManager.HasElement(TraitNames.Evil)))
					{
						if (actor != null)
						{
							foreach (Sim sim3 in lotCurrent.GetSims())
							{
								...
								{
									Relationship relationship5 = Relationship.Get(actor.SimDescription, sim3.SimDescription, true);
									if (relationship5 != null)
									{
										relationship5.MakeAcquaintances();
										Relationship relationship6 = Relationship.Get(actor.SimDescription, sim3.SimDescription, true);
		...
		{
			bool flag = false;
			if (sim.TraitManager.HasElement(TraitNames.SuperVampire))
			{
				flag = true;
			}
