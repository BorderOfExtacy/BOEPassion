...
namespace S3_Passion.BOE_Interaction
{
	internal sealed class CumOnFace : ImmediateInteraction<Sim, Sim>
	{
		...
		private sealed class Definition : ImmediateInteractionDefinition<Sim, Sim, CumOnFace>
		{
			...
			{
				if (PassionBase.CumInteractions)
				{
					return true;
				}
				return false;
		...
		public override bool Run()
		{
			...
			{
				SimMessage(Localization.LocalizeString("You want me to ejaculate on my face? Come close to the screen. I prefer yours!"), Actor);
			}
			...
			if ((Actor.IsMale && Actor != Target && !PersistableSettings.Settings.UseCondom) || (Actor.IsMale && Actor != Target && PersistableSettings.Settings.UseCondom && PersistableSettings.Settings.CondomIsBroken) || (IsTransfem(Actor) && !IsTransfem(Target) && !PersistableSettings.Settings.UseCondom) || (IsTransfem(Actor) && !IsTransfem(Target) && PersistableSettings.Settings.UseCondom && PersistableSettings.Settings.CondomIsBroken))
			{
			...
			if ((Actor.IsFemale && !IsTransfem(Actor) && Target.IsMale && !PersistableSettings.Settings.UseCondom) || (Actor.IsFemale && !IsTransfem(Actor) && Target.IsMale && PersistableSettings.Settings.UseCondom && PersistableSettings.Settings.CondomIsBroken) || (Actor.IsFemale && !IsTransfem(Actor) && IsTransfem(Target) && !PersistableSettings.Settings.UseCondom) || (Actor.IsFemale && !IsTransfem(Actor) && IsTransfem(Target) && PersistableSettings.Settings.UseCondom && PersistableSettings.Settings.CondomIsBroken))
			{
