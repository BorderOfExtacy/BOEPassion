...
namespace S3_Passion.BOE_Interaction
{
	public class CastSexCharm : MagicWand.CastSpell
	{
		...
		public override void OnSpellSuccess()
		{
			...
			Player player2 = PassionBase.GetPlayer(Actor);
            PersistableSettings.Settings.AutonomyNotify = true;
			Target.InteractionQueue.AddAfterCheckingForDuplicates(Interactions.UseSimForPassion.Singleton.CreateInstance(Actor, Target, new InteractionPriority(InteractionPriorityLevel.RequiredNPCBehavior), false, true));
			...
			if (PersistableSettings.Settings.AutonomyChance <= 0 || !PersistableSettings.Settings.AutonomyNotify)
			{
				return;
			}
