...
namespace S3_Passion.BOE_STD
{
	public class STD : CustomBuff
	{
		...
		{
			return sim == null || (PersistableSettings.Settings.STDSimmunity == STDImmunity.Immune && sim.HasTrait(TraitNames.Simmunity));
		}
		...
		public static bool InfectionRandomization(Sim sim, BuffNames std)
		{
			...
			if (sim != null)
			{
				if (PersistableSettings.Settings.STDSimmunity == STDImmunity.Resistant && sim.HasTrait(TraitNames.Simmunity))
				{
					switch (std)
					{
					case (BuffNames)14409719855781998876uL:
		...
		{
			Process(PassionBase.GetPlayer(sim));
		}

		public static void Process(Player player)
		{
			if (player != null && player.IsValid && PersistableSettings.Settings.STD && !SimmunityPreventsInfection(player.Actor))
			{
				try
				{
					Sim actor = player.Actor;
					Part part = player.Part;
					if (part != null)
					{
		...
		public static void AddRandomToAll(bool includeActive, bool messages)
		{
			...
			{
				if (sim == null || !sim.IsHuman || (!includeActive && sim.Household.IsActive) || !PassionBase.IsValidAge(sim) || SimmunityPreventsInfection(sim))
				{
					continue;
				}
				bool flag = false;
		...
		public static void RemoveFromAll()
		{
			...
			foreach (Sim sim in array)
			{
				if (sim == null || !sim.IsHuman || !PassionBase.IsValidAge(sim, true))
				{
					continue;
				}
				foreach (BuffNames item in Names.STD)
