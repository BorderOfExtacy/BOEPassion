...
namespace S3_Passion.BOE_Lovemaking
{
	...
	public class PassionType
	{
		...
		public static void Load(Type type, bool use, bool parts)
		{
			...
			if (PassionBase.LoadedTypes.ContainsKey(type.Name))
			{
				if (PassionBase.LoadedTypes[type.Name] == null)
				{
					PassionBase.LoadedTypes[type.Name] = Create(type, use, parts);
				}
			}
			else
			{
				PassionBase.LoadedTypes.Add(type.Name, Create(type, use, parts));
			}
		}
		...
		{
			if (PassionBase.LoadedTypes.ContainsKey(name))
			{
				PassionBase.LoadedTypes.Remove(name);
			}
		}

		public static void Unload()
		{
			PassionBase.LoadedTypes.Clear();
		}
		...
		public static bool IsLoaded(string name)
		{
			...
			{
				return PassionBase.LoadedTypes.ContainsKey(name) && PassionBase.LoadedTypes[name] != null;
			}
			return false;
		}
		...
		public static PassionType GetSupportedType(GameObject obj)
		{
			...
			if (obj != null)
			{
				...
				else
				{
					...
					if (passionType == null)
					{
						foreach (PassionType value in PassionBase.LoadedTypes.Values)
						{
		...
		{
			if (type != null && PassionBase.LoadedTypes.ContainsKey(type.Name))
			{
				return PassionBase.LoadedTypes[type.Name];
			}
			return null;
		}
		...
		{
			if (!string.IsNullOrEmpty(name) && PassionBase.LoadedTypes.ContainsKey(name))
			{
				return PassionBase.LoadedTypes[name];
			}
			return null;
		}
