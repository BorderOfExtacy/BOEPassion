...
namespace Passion.S3_Passion
{
    public class Std : CustomBuff
    {
        public class Infection
        {
            ...
            public Sim Carrier
            {
                get { return _carrierDescription != null ? _carrierDescription.CreatedSim : null; }
            ...
            private void Initialize(Sim carrier, BuffNames disease, long onset, bool reinfection)
            {
                ...
                IsValid = true;
                _incubationAlarm = Carrier.AddAlarmRepeating(5f, TimeUnit.Minutes, Callback, 7f, TimeUnit.Hours,
                    "Periodic STD Check", AlarmType.AlwaysPersisted);
            }
            ...
            private void Callback()
            {
                ...
                if (disease == (BuffNames)3348796355468607868uL || disease == (BuffNames)3802937897211410794uL ||
        ...
        public class DeathClock
        {
            ...
            public DeathClock(Sim carrier, BuffNames disease)
            {
                ...
                SetNextCheck();
                IsValid = true;
                _deathClockAlarm = Carrier.AddAlarmRepeating(1f, TimeUnit.Days, Callback, 5f, TimeUnit.Hours,
                    "DeathClock Alarm", AlarmType.AlwaysPersisted);
            }
            ...
            private void Callback()
            {
                ...
                {
                    case (BuffNames)3348796355468607868uL:
                        if (Carrier != null && !Carrier.TraitManager.HasElement(TraitNames.Insane) &&
                            RandomUtil.RandomChance(20f))
                        {
                            try
                            {
        ...
        public class GetTreated : RabbitHole.RabbitHoleInteraction<Sim, RabbitHole>
        {
            ...
            {
                public override bool Test(Sim sim, RabbitHole target, bool isAutonomous,
                ...
                {
                    return Localization.LocalizeString("STD.GetTreated:InteractionName") + "(" +
                           UIUtils.FormatMoney(_kCostOfTreatment) + ")";
                }
            }
            ...
            public override void ConfigureInteraction()
            {
                ...
                TimedStage timedStage =
                    new TimedStage(GetInteractionName(), _kSimMinutesForTreatment, false, true, true);
        ...
        [PersistableStatic] private static List<Infection> _mPendingInfections;

        public static List<Infection> PendingInfectionsBackup = new List<Infection>();

        [PersistableStatic] private static List<DeathClock> _mDeathClocks;
        ...
        private static bool HasSiv(Sim sim)
        {
            foreach (BuffNames sTd in GetStDs(sim))
            {
                ...
                if (buffNames != (BuffNames)3802937897211410794uL && buffNames != (BuffNames)6514717556347855497uL &&
                    buffNames != (BuffNames)16680654480998469135uL)
                {
                    continue;
                }
            ...
            foreach (Infection pendingInfection in PendingInfections)
            {
                ...
                if (disease != (BuffNames)3802937897211410794uL && disease != (BuffNames)6514717556347855497uL &&
                    disease != (BuffNames)16680654480998469135uL)
                {
                    continue;
                }
        ...
        {
            return sim == null || (PassionCore.Settings.StdSimmunity == StdImmunity.Immune &&
                                   sim.HasTrait(TraitNames.Simmunity));
        }
        ...
        private static bool InfectionRandomization(Sim sim, BuffNames std)
        {
            ...
            if (PassionCore.Settings.StdSimmunity == StdImmunity.Resistant && sim.HasTrait(TraitNames.Simmunity))
            {
                switch (std)
                {
                    case (BuffNames)14409719855781998876uL:
        ...
        private static void Process(PassionCore.Player player)
        {
            if (player != null && player.IsValid && PassionCore.Settings.Std &&
                !SimmunityPreventsInfection(player.Actor))
            {
                try
                {
                    Sim actor = player.Actor;
                    PassionCore.Part part = player.Part;
                    if (part != null)
                    {
                        foreach (PassionCore.Player value in part.Players.Values)
                        {
                            ...
                            try
                            {
                                foreach (BuffNames sTd in GetStDs(value.Actor))
                                {
                                    switch (sTd)
                                    {
                                        case (BuffNames)12465744095023444500uL:
                                        case (BuffNames)16328498947224254690uL:
                                            if (InfectionRandomization(actor, sTd) &&
                                                !actor.BuffManager.HasElement(sTd))
                                            {
                                        ...
                                        case (BuffNames)9183286871710270222uL:
                                            ...
                                            {
                                                PendingInfections.Add(new Infection(actor,
                                                    (BuffNames)9183286871710270222uL,
                                                    GetOnset((BuffNames)9183286871710270222uL)));
                                            }
                                        ...
                                        case (BuffNames)6514717556347855497uL:
                                            ...
                                            {
                                                PendingInfections.Add(new Infection(actor,
                                                    (BuffNames)3802937897211410794uL,
                                                    GetOnset((BuffNames)3802937897211410794uL)));
                                            }
                                        ...
                                        case (BuffNames)8648471817152268810uL:
                                            ...
                                            {
                                                PendingInfections.Add(new Infection(actor,
                                                    (BuffNames)14409719855781998876uL,
                                                    GetOnset((BuffNames)14409719855781998876uL)));
                                            }
                            ...
                            try
                            {
                                foreach (Infection pendingInfection in PendingInfections)
                                {
                                    ...
                                    switch (pendingInfection.Disease)
                                    {
                                        case (BuffNames)12465744095023444500uL:
                                        case (BuffNames)16328498947224254690uL:
                                            if (InfectionRandomization(actor, pendingInfection.Disease) &&
                                                !actor.BuffManager.HasElement(pendingInfection.Disease))
                                            {
                                                PendingInfections.Add(new Infection(actor, pendingInfection.Disease,
                                                    GetOnset(pendingInfection.Disease)));
                                            }
        ...
        private static void Progression(BuffManager bm, BuffInstance bi)
        {
            ...
            {
                case (BuffNames)12465744095023444500uL:
                    PendingInfections.Add(new Infection(bm.Actor, (BuffNames)12465744095023444500uL,
                        SimClock.CurrentTicks + DayTicks * RandomUtil.GetInt(30, 45), false));
                    break;
                case (BuffNames)9183286871710270222uL:
                    PendingInfections.Add(new Infection(bm.Actor, (BuffNames)4642255024717194076uL,
                        GetOnset((BuffNames)4642255024717194076uL)));
                    break;
                case (BuffNames)4642255024717194076uL:
                    PendingInfections.Add(new Infection(bm.Actor, (BuffNames)3348796355468607868uL,
                        GetOnset((BuffNames)3348796355468607868uL)));
                    break;
                case (BuffNames)6514717556347855497uL:
                ...
                case (BuffNames)14409719855781998876uL:
                case (BuffNames)8648471817152268810uL:
                    PendingInfections.Add(new Infection(bm.Actor,
                        RandomUtil.RandomChance(15f)
                            ? ((BuffNames)14409719855781998876uL)
                            : ((BuffNames)8648471817152268810uL), GetOnset((BuffNames)8648471817152268810uL)));
                    break;
                default:
        ...
        private static void AddRandomToAll(bool includeActive, bool messages)
        {
            ...
            {
                if (sim == null || !sim.IsHuman || (!includeActive && sim.Household.IsActive) ||
                    !PassionCore.IsValidAge(sim) || SimmunityPreventsInfection(sim))
                {
                    continue;
                }
                ...
                if (RandomUtil.RandomChance(15f) && !sim.BuffManager.HasElement((BuffNames)12465744095023444500uL))
                {
                    if (messages)
                    {
                        PassionCommon.BufferLine("  " +
                                                 PassionCommon.Localize("Gameplay/Excel/buffs/BuffList:lSTDSimydia"));
                    }
                    ...
                    if (messages)
                    {
                        PassionCommon.BufferLine("  " +
                                                 PassionCommon.Localize(
                                                     "Gameplay/Excel/buffs/BuffList:lSTDSimnorrhea"));
                    }
                ...
                if (RandomUtil.RandomChance(15f) && !HasSimerpes(sim))
                {
                    ...
                    {
                        PassionCommon.BufferLine("  " +
                                                 PassionCommon.Localize("Gameplay/Excel/buffs/BuffList:lSTDSimerpes"));
                    }
                ...
                if (RandomUtil.RandomChance(15f) && !HasSimphilis(sim))
                {
                    ...
                    {
                        PassionCommon.BufferLine("  " +
                                                 PassionCommon.Localize("Gameplay/Excel/buffs/BuffList:lSTDSimphilis"));
                    }
                ...
                if (RandomUtil.RandomChance(5f) && !HasSiv(sim))
                {
                    ...
                    {
                        PassionCommon.BufferLine("  " +
                                                 PassionCommon.Localize("Gameplay/Excel/buffs/BuffList:lSTDSIV"));
                    }
        ...
        public static void RemoveFromAll()
        {
            ...
            foreach (Sim sim in array)
            {
                if (sim == null || !sim.IsHuman || !PassionCore.IsValidAge(sim, true))
                {
                    continue;
                }
            ...
            DeathClocks.Clear();
            PassionCommon.SystemMessage("Removed " + num + " diseases, " + num2 + " incubating infections, and " +
                                        num3 + " active death clocks.");
        }
        ...
        public static void RemoveFrom(Sim carrier)
        {
            ...
            foreach (DeathClock deathClock in DeathClocks)
            {
                if (deathClock.Carrier == carrier)
                {
                    deathClock.Invalidate();
                }
            }
        }
