...
namespace Passion.S3_Passion.Interactions
{
    ...
    {
        public sealed class Definition : InteractionDefinition<Sim, Terrain, VampireDrainsEnergyFromAll>, IHasTraitIcon,
            IHasMenuPathIcon
        {
                ...
                ref GreyedOutTooltipCallback greyedOutTooltipCallback)
            {
                ...
                if (Terrain.GoHere.SharedGoHereTests(ref parameters))
                {
                    ...
                    {
                        if (!sim2.SimDescription.IsRobot && sim2 != parameters.Actor && flag &&
                            !sim2.SimDescription.IsEP11Bot && !sim2.SimDescription.ChildOrBelow &&
                            !sim2.SimDescription.IsBonehilda && !sim2.SimDescription.IsPet &&
                            sim2.RoomId == parameters.Actor.RoomId && !parameters.Actor.LotCurrent.IsWorldLot)
                        {
                            num++;
                        }
                    }
                ...
                foreach (Sim sim in lotCurrent.GetSims())
                {
                    if (sim != Actor && sim.SimDescription.TeenOrAbove && sim.RoomId == Actor.RoomId)
                    {
                        ...
                        ThoughtBalloonManager.BalloonData balloonData =
                            new ThoughtBalloonManager.BalloonData(sim.GetThumbnailKey());
