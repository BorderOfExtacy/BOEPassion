...
{
    internal sealed class CumOnLeftHand : ImmediateInteraction<Sim, Sim>
    {
        [DoesntRequireTuning]
        private sealed class Definition : ImmediateInteractionDefinition<Sim, Sim, CumOnLeftHand>
        {
            public override string GetInteractionName(Sim actor, Sim target, InteractionObjectPair interaction)
            {
                return Localization.LocalizeString("CumOnLeftHand");
            }
        ...
        {
            if (Actor.IsFemale && Target.IsFemale && !IsTransfem(Actor))
            {
                SimMessage(Localization.LocalizeString("I don't have the ability to do that..."), Actor);
            }

            if ((Actor.IsMale && PassionCore.Settings.UseCondom && PassionCore.Settings.CondomIsBroken) ||
                (Actor.IsMale && !PassionCore.Settings.UseCondom) ||
                (IsTransfem(Actor) && PassionCore.Settings.UseCondom && PassionCore.Settings.CondomIsBroken) ||
                (IsTransfem(Actor) && !PassionCore.Settings.UseCondom))
            {
                ...
                ResourceKey resourceKey2 = ResourceKey.FromString("0x034AEECB-0x00000000-0x0314BC45425F8C5F");
                SimDescription simDescription2 = Target.SimDescription;
                SimOutfit simOutfit2 = null;
                if (resourceKey2 != kInvalidResourceKey)
                {
                    simOutfit2 = new SimOutfit(ResourceKey.FromString("0x025ED6F4-0x00000000-0xFC06B5A2D8D77A70"));
                }
            ...
            ResourceKey resourceKey4 = ResourceKey.FromString("0x034AEECB-0x00000000-0x0314BC45425F8C5F");
            SimDescription simDescription4 = Actor.SimDescription;
            SimOutfit simOutfit4 = null;
            if (resourceKey4 != kInvalidResourceKey2)
            {
                simOutfit4 = new SimOutfit(ResourceKey.FromString("0x025ED6F4-0x00000000-0xFC06B5A2D8D77A70"));
            }

            if (simOutfit4 != null && Actor.CurrentOutfitCategory == OutfitCategories.Naked)
            {
                SimOutfit resultOutfit4;
                if (OutfitUtils.TryApplyUniformToOutfit(simDescription4.GetOutfit(OutfitCategories.Naked, 0),
                        simOutfit4, simDescription4, "CumMask", out resultOutfit4))
                {
