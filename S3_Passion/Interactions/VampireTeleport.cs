...
namespace Passion.S3_Passion.Interactions
{
    ...
    {
        private sealed class Definition : InteractionDefinition<Sim, Terrain, VampireTeleport>, IHasTraitIcon,
            IHasMenuPathIcon
        {
                ...
                ref GreyedOutTooltipCallback greyedOutTooltipCallback)
            {
                ...
                return actor.SimDescription.IsVampire && GameUtils.IsInstalled(ProductVersion.EP7) &&
        ...
        {
            Sim actor = Actor;
            if (!actor.Posture.Satisfies(CommodityKind.SwimmingInPool, null) &&
                !actor.Posture.Satisfies(CommodityKind.KeepSwimming, null))
            {
            ...
            const string effectName = "ep3vampiretransition";
        ...
        public static void TeleportVamp(Sim actor, Vector3 destination, ITeleporter teleporter, bool bPlayVfXvamp,
            bool fadeSim)
        {
            ...
            if (bPlayVfXvamp)
            {
                ...
                visualEffect.SetPosAndOrient(actor.Position, actor.ForwardVector, actor.UpVector);
                visualEffect.SubmitOneShotEffect(VisualEffect.TransitionType.SoftTransition);
                actor.PlaySoloAnimation(actor.SimDescription.Child ? "c_floating_x" : "a_floating_x", false);
            }
