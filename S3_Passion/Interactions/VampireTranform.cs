...
namespace Passion.S3_Passion.Interactions
{
    ...
    {
        [DoesntRequireTuning]
        private sealed class Definition : InteractionDefinition<Sim, Sim, VampireTranform>, IHasTraitIcon,
            IHasMenuPathIcon
        {
            ...
            {
                return Localization.LocalizeString(target.IsFemale, "Vampire Transform", target.SimDescription,
            ...
            {
                return GameUtils.IsInstalled(ProductVersion.EP7) && target != a &&
                       a.TraitManager.HasElement(TraitNames.SuperVampire) && !target.IsActiveSim &&
                       ...
                       !target.SimDescription.IsPet && !target.SimDescription.IsVampire && a.SimDescription.IsVampire &&
                       !a.SimDescription.TeenOrBelow && !a.Posture.Satisfies(CommodityKind.SwimmingInPool, null) &&
        ...
        public override bool Run()
        {
            ...
            EventTracker.SendEvent(new VampireLifetimeEvent(EventTypeId.kVampireLifetimeEvent, Actor.SimDescription,
                true, Target.SimDescription.SimDescriptionId));
            Cleanup();
            return true;
        }
        ...
        private static void VampVictBuffs(Sim vic)
        {
        ...
        private static void VampireDrinkSuccess(Sim actor)
        {
