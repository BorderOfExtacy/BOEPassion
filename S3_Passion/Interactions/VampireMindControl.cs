...
namespace Passion.S3_Passion.Interactions
{
    ...
    {
        [DoesntRequireTuning]
        private sealed class Definition : InteractionDefinition<Sim, Sim, VampireMindControl>, IHasTraitIcon,
            IHasMenuPathIcon
        {
            ...
            {
                return Localization.LocalizeString(target.IsFemale, "Vampire Mind Control", target.SimDescription,
                ...
                ref GreyedOutTooltipCallback greyedOutTooltipCallback)
            {
                return GameUtils.IsInstalled(ProductVersion.EP7) &&
                       actor.TraitManager.HasElement(TraitNames.SuperVampire) && actor.SimDescription.IsVampire &&
                       !actor.Posture.Satisfies(CommodityKind.SwimmingInPool, null) &&
                       !target.Posture.Satisfies(CommodityKind.SwimmingInPool, null) &&
                       !target.SimDescription.IsRobot && target != actor && !target.SimDescription.IsEP11Bot &&
                       target.Household != actor.Household && !target.BuffManager.HasElement(BuffNames.WeddingDay) &&
                       !target.SimDescription.IsPregnant && !target.BuffManager.HasElement(BuffNames.Ensorcelled) &&
                       !target.SimDescription.IsServicePerson && !target.SimDescription.ChildOrBelow &&
