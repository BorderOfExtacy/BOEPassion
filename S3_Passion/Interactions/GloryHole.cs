...
namespace Passion.S3_Passion.Interactions
{
    public class GloryHole : PassionCommon
    {
        ...
        public class ActiveGloryHole
        {
            ...
            public bool HasSlut
            {
                get { return Slut != null; }
            }

            public bool HasStud
            {
                get { return Stud != null; }
            }

            public bool HasBoth
            {
                get { return HasStud && HasSlut; }
            }

            public bool BothReady
            {
                get { return SlutReady && StudReady; }
            }
        ...
        public class ServiceStrangers : Interaction<Sim, FenceRedwood_Gate>
        {
            ...
            {
                public override string GetInteractionName(Sim actor, FenceRedwood_Gate target,
                    InteractionObjectPair interaction)
                {
                    return PassionCommon.Localize("S3_Passion.Terms.ServiceStrangers").ToString();
                }

                public override bool Test(Sim actor, FenceRedwood_Gate target, bool isAutonomous,
                    ...
                    InteractionObjectPair interaction)
                {
                    ...
                    if (activeGloryHole != null)
                    {
                        return PassionCommon.Localize("S3_Passion.Terms.UseGloryHole") + ": " +
                               UIUtils.FormatMoney(ActiveGloryHole.Cost);
                    }

                    return PassionCommon.Localize("S3_Passion.Terms.UseGloryHole").ToString();
                }
                       ...
                       !Actor.Motives.CheckMotivesForTimeToLeave(Actor.Motives, this, false, Autonomous))
                {
                    ...
                    if (activeGloryHole.BothReady)
                    {
                        if (!activeGloryHole.CommodityUpdates)
                        {
                            BeginCommodityUpdate(
                                new CommodityChange(CommodityKind.Fun, 300f, false, 300f, OutputUpdateType.First, false,
                                    true, UpdateAboveAndBelowZeroType.Either), 1f);
                            activeGloryHole.Slut.CurrentInteraction.BeginCommodityUpdate(
                                new CommodityChange(CommodityKind.Social, 50f, false, 50f, OutputUpdateType.First,
                                    false, true, UpdateAboveAndBelowZeroType.Either), 1f);
                            activeGloryHole.Slut.CurrentInteraction.BeginCommodityUpdate(
                                new CommodityChange(CommodityKind.Hygiene, -50f, false, -50f, OutputUpdateType.First,
                                    false, true, UpdateAboveAndBelowZeroType.Either), 1f);
                            if (Actor.HasTrait(TraitNames.PartyAnimal) || Actor.HasTrait(TraitNames.Adventurous) ||
                                Actor.HasTrait(TraitNames.Slob))
                            {
                                activeGloryHole.Slut.CurrentInteraction.BeginCommodityUpdate(
                                    new CommodityChange(CommodityKind.Fun, 150f, false, 150f, OutputUpdateType.First,
                                        false, true, UpdateAboveAndBelowZeroType.Either), 1f);
                            }
                    ...
                    if (activeGloryHole.AnimationSwitch)
                    {
                        ...
                        if (activeGloryHole.HasSlut && Actor.Motives.GetMotiveValue(CommodityKind.Bladder) < -80f)
                        {
                            ...
                            activeGloryHole.Slut.Motives.LerpToFill(activeGloryHole.Slut.CurrentInteraction,
                                CommodityKind.Hunger, 2f);
                        }
                    }
