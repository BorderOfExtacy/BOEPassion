...
{
    public abstract class CustomDance : Interaction<Sim, DanceFloor>
    {
        public class NightFever : Interaction<Sim, DanceFloor>
        {
            [DoesntRequireTuning]
            private sealed class Definition : InteractionDefinition<Sim, DanceFloor, NightFever>
            {
                public override string GetInteractionName(Sim actor, DanceFloor target,
                    InteractionObjectPair interaction)
                {
                    return Localization.LocalizeString("S3_Passion.Terms.NightFever");
                }

                public override string[] GetPath(bool isFemale)
                {
                    return new string[] { Localization.LocalizeString("CustomDance") };
                }

                public override bool Test(Sim actor, DanceFloor target, bool isAutonomous,
                    ref GreyedOutTooltipCallback greyedOutTooltipCallback)
                {
                    return !isAutonomous && actor != null && target != null && actor.SimDescription.TeenOrAbove;
                }
            }
            ...
            {
                List<Stereo> list =
                    new List<Stereo>(global::Sims3.Gameplay.Queries.GetObjects<Stereo>(Actor.LotCurrent, Actor.RoomId));
                Stereo stereo = null;
                ...
                while (stereo != null && Actor.HasNoExitReason() &&
                       !Actor.Motives.CheckMotivesForTimeToLeave(Actor.Motives, (InteractionInstance)this, false,
                           Autonomous))
                {
                    ...
                    Actor.PlaySoloAnimation(RandomUtil.GetRandomObjectFromList(NightFeverAnimations));
        ...
        private static readonly List<string> NightFeverAnimations;

        private static readonly List<string> DiscoAnimations;

        private static readonly List<string> HifiraverAnimations;

        private static readonly List<string> JunjoufFighterAnimations;

        private static readonly List<string> TiktTokAnimations;

        private static readonly List<string> ValentiAnimations;

        static CustomDance()
        {
            List<string> list = new List<string>();
            list.Add("a_umpa_simsnightfever");
            NightFeverAnimations = list;
